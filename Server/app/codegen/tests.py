...
class CodegenTests(TestCase):
    ...
    def test_code_gen_check(self):
        ...
        self.assertTrue(self.base_codegen.check_token(self.test_user,self.base_code))
    ...
    def test_code_timeout(self):
        ...
        endtime_codegen = TestCodeGen(datetime.now() + timedelta(seconds=settings.CODE_TIMEOUT))
        self.assertTrue(endtime_codegen.check_token(self.test_user,self.base_code))

        expired_codegen = TestCodeGen(datetime.now()+ timedelta(seconds=settings.CODE_TIMEOUT + 1))
        self.assertFalse(expired_codegen.check_token(self.test_user, self.base_code))
